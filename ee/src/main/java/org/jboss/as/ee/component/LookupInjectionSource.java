...
public final class LookupInjectionSource extends InjectionSource {
    ...
    public void getResourceValue(final ResolutionContext resolutionContext, final ServiceBuilder<?> serviceBuilder, final DeploymentPhaseContext phaseContext, final Injector<ManagedReferenceFactory> injector) {
        ...
        if (lookupName.isJava()) {
            ...
            if (resolutionContext.getDeploymentBindings().contains(lookupName)) {
                // deployment doing the referred binding, setup a mandatory dependency to the binder
                serviceBuilder.addDependency(bindInfo.getBinderServiceName());
            } else {
                // set an optional dependency to the binder or its parent external context's service name
                ...
                serviceBuilder.addDependency(ServiceBuilder.DependencyType.OPTIONAL, dependencyServiceName);
            }
