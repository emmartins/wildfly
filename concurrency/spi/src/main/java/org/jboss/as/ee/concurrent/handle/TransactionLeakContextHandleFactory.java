...
public class TransactionLeakContextHandleFactory implements ContextHandleFactory {
    ...
    private static class TransactionLeakSetupContextHandle implements SetupContextHandle {
        ...
        private void writeObject(ObjectOutputStream out) throws IOException {
            throw ConcurrencyLogger.ROOT_LOGGER.serializationMustBeHandledByTheFactory();
        }
        ...
        public void reset() {
            if(transactionManager != null) {
                try {
                    ...
                    if(transactionOnReset != null && !transactionOnReset.equals(transactionOnSetup)) {
                        switch (transactionOnReset.getStatus()) {
                            ...
                            case Status.STATUS_PREPARED:
                                try {
                                    ConcurrencyLogger.ROOT_LOGGER.rollbackOfTransactionStartedInEEConcurrentInvocation();
                                    transactionManager.rollback();
                                } catch (Throwable e) {
                                    ConcurrencyLogger.ROOT_LOGGER.failedToRollbackTransaction(e);
                                } finally {
                                    try {
                                        transactionManager.suspend();
                                    } catch (Throwable e) {
                                        ConcurrencyLogger.ROOT_LOGGER.failedToSuspendTransaction(e);
                                    }
                                }
                        }
                    }
                    ...
                    ConcurrencyLogger.ROOT_LOGGER.systemErrorWhileCheckingForTransactionLeak(e);
                }
            }
        }
