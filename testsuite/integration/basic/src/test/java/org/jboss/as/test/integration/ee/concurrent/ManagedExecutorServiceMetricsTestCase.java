...
public class ManagedExecutorServiceMetricsTestCase {
    ...
    public void testManagedExecutorServiceManagement() throws Exception {
        ...
        try {
            ...
            testRuntimeStats(pathAddress, executorService, true);
        } finally {
            // remove
    ...
    public void testManagedScheduledExecutorServiceManagement() throws Exception {
        ...
        try {
            ...
            final ManagedExecutorService executorService = InitialContext.doLookup(jndiName);
            Assert.assertNotNull(executorService);
            testRuntimeStats(pathAddress, executorService, false);
        } finally {
            // remove
            final ModelNode removeOperation = Util.createRemoveOperation(pathAddress);
            removeOperation.get(OPERATION_HEADERS, ALLOW_RESOURCE_SERVICE_RESTART).set(true);
    ...
    private void testRuntimeStats(PathAddress pathAddress, ManagedExecutorService executorService, boolean testQueueSize) throws IOException, ExecutionException, InterruptedException, BrokenBarrierException {
        ...
        executorService.submit(() -> {
            ...
            // after sleeping for hung threshold time the thread executing this should be considered hung...
            try {
                Assert.assertTrue(readStatsAttribute(pathAddress, ManagedExecutorServiceMetricsAttributes.HUNG_THREAD_COUNT) == 1);
            } catch (IOException e) {
                Assert.fail();
            }
        }).get();
        ...
        // resume tasks running and confirm expected idle stats
        barrier2.await();
        Assert.assertTrue(readStatsAttribute(pathAddress, ManagedExecutorServiceMetricsAttributes.ACTIVE_THREAD_COUNT) == 0);
